      ...
      <Card>
        ...
        <div className="overflow-x-auto mt-2">
          <table className="w-full text-sm">
            ...
            <tbody>
              {lista.map((f:any)=>(
                <tr key={f.id} className="border-t"><td>{f.tipo}</td><td className="text-center">{f.referencia}</td><td className="text-xs" title={f.unidadeId}>{f.unidade?.identificacao || unidades.find((u:any)=>u.id===f.unidadeId)?.identificacao || f.unidadeId} </td><td className="text-xs">{f.inquilino?.nome ? <span className="font-medium">{f.inquilino.nome}</span> : <span className="text-amber-600 font-semibold">— integral</span>}</td><td className="text-[10px] max-w-[160px] truncate" title={f.criterioRateio}>{f.criterioRateio||"-"}</td><td className="text-xs text-center">{f.tipo==="ENERGIA" ? <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${f.bandeira==="VERDE"?"bg-emerald-100 text-emerald-700":f.bandeira==="AMARELA"?"bg-amber-100 text-amber-700":f.bandeira?.startsWith("VERMELHA")?"bg-rose-100 text-rose-700":"bg-zinc-100"}`}>{f.bandeira||"VERDE"}</span> : "-"}</td><td className="text-right">{brl(f.valorTotal)}{f.rateioValor && f.rateioValor!==f.valorTotal ? <div className="text-[10px] text-zinc-500">{brl(f.rateioValor)}</div> : null}</td><td className="text-right text-xs">{f.valorDemonstrativo ? brl(f.valorDemonstrativo) : "-"}<div className="text-[10px] text-zinc-500 truncate max-w-[120px]">{f.descricaoDemonstrativo||""}</div></td><td className="text-center text-xs">{f.tipo==="CONDOMINIO" ? <span className="text-emerald-600">Sempre</span> : f.exibirDemonstrativo===false ? <span className="text-zinc-500">Desativado</span> : <span className="text-emerald-600">Ativo</span>}</td><td className="text-xs text-center">{new Date(f.dataVencimento).toLocaleDateString("pt-BR")}</td><td className="text-center"><Badge variant={f.status==="PAGA"?"success":f.status==="VENCIDA"?"danger":"default"}>{f.status}</Badge></td>                    <td className="text-right space-x-1"><div className="flex flex-col gap-1 items-end">{!f.inquilinoId && ["ENERGIA","AGUA","GAS"].includes(f.tipo) && <button onClick={()=>recalcularRateio(f.id)} className="text-[10px] bg-amber-500 text-white rounded-full px-2 py-0.5">↻ Ratear {brl(f.valorTotal)}</button>}<div className="flex gap-1"><button onClick={()=>iniciarEdicao(f)} className="text-xs bg-emerald-700 text-white rounded-full px-3 py-1">Alterar</button><button onClick={()=>remover(f.id)} className="text-xs text-rose-600">Remover</button></div></div></td></tr>
              ))}
