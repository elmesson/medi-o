...
export default function FaturasPdfPage(){
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-zinc-500">Visualize todas as faturas em PDF de determinada Unidade, filtrando por Referência (ex: 2026-08) e Unidade.</p>

      <Card className="space-y-3">
        <div className="grid md:grid-cols-4 gap-3">
          <label className="text-sm">Unidade<select value={filtroUnidade} onChange={e=>setFiltroUnidade(e.target.value)} className="mt-1 w-full border rounded-xl px-3 py-2"><option value="">Todas as unidades</option>{unidades.map((u:any)=><option key={u.id} value={u.id}>{u.identificacao}</option>)}</select></label>
          ...
          <div className="flex items-end gap-2">
            ...
            <button onClick={()=>{setFiltroUnidade(""); setReferencia("2026-08"); setTipo("TODOS");}} className="flex-1 border rounded-xl py-2 text-sm">Limpar</button>
          </div>
        </div>
      ...
      <Card>
        ...
        <div className="overflow-x-auto mt-3">
          <table className="w-full text-sm">
            <thead><tr className="text-xs text-zinc-500"><th className="text-left">Unidade</th><th>Tipo</th><th>Ref</th><th className="text-right">Valor</th><th>Status</th><th>Vencimento</th><th className="text-right">PDF</th></tr></thead>
            <tbody>
              {filtrados.map((f:any)=>(
              ...
              {filtrados.length===0 && <tr><td colSpan={7} className="text-center py-6 text-zinc-500">Nenhuma fatura para {referencia}{filtroUnidade ? ` na unidade ${unidades.find((u:any)=>u.id===filtroUnidade)?.identificacao||filtroUnidade}` : ""}</td></tr>}
            </tbody>
          </table>
        </div>
      </Card>
